...
class CompressServiceV2(
    ...
    fun compressImage(
        ...
        logger.info("Starting fast in-memory compression for ${imageRequestData.originalFormat}")
            ...
            when (imageRequestData.originalFormat.lowercase()) {
                "png" -> {
                    if (imageRequestData.rawBytes != null) {
                        ...
                        fastPngCompressionService.compressPng(tempFile, compressionType).also { tempFile.delete() }
                    }
                }
                ...
                "jpeg", "jpg" -> {
                    if (imageRequestData.rawBytes != null) {
                        ...
                        val tempFile = createTempFile(imageRequestData)
                        fastJpegCompressionService.compressJpegInMemory(tempFile, compressionType)
                ...
                "webp" -> {
                    if (imageRequestData.rawBytes != null) {
                        ...
                        val tempFile = createTempFile(imageRequestData)
                        fastWebPCompressionService.compressWebPInMemory(tempFile, compressionType)
                            .also { tempFile.delete() }
                    }
                }
                // Fallback to file-based processing for complex formats
                "gif" -> {
                    val rawBytes =
                        imageRequestData.rawBytes ?: throw IllegalArgumentException("Raw bytes required for GIF")
                    gifCompressionService.compressGifUsingGifsicle(rawBytes, compressionType)
                }

                "svg", "svg+xml" -> {
                    val rawBytes =
                        imageRequestData.rawBytes ?: throw IllegalArgumentException("Raw bytes required for SVG")
        ...
        logger.info("Fast compression complete: ${imageRequestData.originalFileSize} B → ${compressedBytes.size} B (${compressionPercentage.toInt()}% reduction, ${duration}ms)")
    ...
    private fun createTempFile(imageRequestData: ImageRequestData): File {
        ...
        } else if (imageRequestData.imageFile != null) {
            javax.imageio.ImageIO.write(imageRequestData.imageFile, imageRequestData.originalFormat, tempFile)
        }
        return tempFile
    }
}